...
/*
Serialized Txn format in bdk
{
    "version": 1,
    "lock_time": 2538538,
    "input": [
        {
            "previous_output": "ca37042c41507326851792fd14a277edad58be9a31f34f92279ae49c78ea536e:0",
            "script_sig": "",
            "sequence": 4294967293,
            "witness": [
                "30440220713e3cc94ca6ede3d260e2988c8e9c401c6994b94426ef7c312fb9a08d87517102203fbbc1c40006bd5129b8a61a6ad5c2b8a0a685cadc0a7e6c944a28e36abda50601",
    ...
    "output": [
        {
            "value": 997359,
            "script_pubkey": "0014338c66cae84cc027c3fb7066e295db3d556d3b0a"
        },
        {
            "value": 2500,
            "script_pubkey": "00149f2bd4d644c40f2ac41e95d339a3f737744b17e1"
        }
    ]
}
*/

class FakeBdkWalletWithTxns extends Fake implements bdk.Wallet {
  FakeBdkWalletWithTxns({this.txCount = 3, this.confs = const []});

  int txCount;
  List<bool> confs;

  @override
  Future<List<bdk.TransactionDetails>> listTransactions(bool includeRaw) async {
    /*
    // unconfirmed tx format
    const bdk.TransactionDetails(
        txid: '480d98fa8fa67709aa002665840bf45bdeed2182d4404fb3c7a9f0627237233a',
        received: 100,
        sent: 200,
        fee: 209,
        serializedTx:
            '{"version":1,"lock_time":2538788,"input":[{"previous_output":"a31d68b9e78eac674958f2e067b4e5297b2588bc62f6f03ab304852818b307d6:0","script_sig":"","sequence":4294967293,"witness":["3044022059a54e7da09985c7c3b2f0ba022f5cd8eae024afab0a6b61db8eb907f5b5ae3302204ce5ad6ab02dfc733ed18288fce5768a49a6d2f95af0abc82ea0a480ac234bea01","02859c3672bef8fa49d9597779e206b7b09f2e33355b9a10624973ce374f0edf2c"]},{"previous_output":"d24f68e181b780724a93cbb40061b20ce1e2ec44fcd33b56e93f564e5002b86c:1","script_sig":"","sequence":4294967293,"witness":["30440220296c553d33e4e9d1cf0e8324dde50977ea5bb2532a3f6ea97cf1f2d3c388d524022054c0723460dcf5af59915b6877cf060a7a984644e5b21722f2993322524f6e2701","0242bae09cf5a5035b05b246512d5ee0f9f773bc888ba79b5233106bf8f35908eb"]}],"output":[{"value":631965,"script_pubkey":"00146f67b3a06f0370acb245997595869235532d7204"},{"value":2500,"script_pubkey":"0014ceada4b239a727cb36c676f1e253f1d33cbe42ec"}]}');
            */

    bdk.TransactionDetails tx1;
    if (confs.isNotEmpty && confs[0]) {
      tx1 = const bdk.TransactionDetails(
    ...
    bdk.TransactionDetails tx2;
    if (confs.length > 1 && confs[1]) {
      tx2 = const bdk.TransactionDetails(
    ...
    bdk.TransactionDetails tx3;
    if (confs.length > 2 && confs[2]) {
      tx3 = const bdk.TransactionDetails(
    ...
    //print(confs.length > 1 ? confs[1] : 'null');
    ...
    // return only number of elements from list based on txCount
    return list.sublist(0, txCount);
  }
}

Wallet getMagicWallet() {
  return const Wallet(
...
Wallet getEmptyWallet() {
  return getMagicWallet();
}

/// Unconfirmed txns have height = 0, timestamp = 0
Wallet getWalletWith3Unconfirmed({List<String> label = const []}) {
  ...
  final Transaction tx1 = Transaction(
    ...
    received: 1000000,
    sent: 0,
    label: label.isNotEmpty ? label[0] : null,
    toAddress: 'tb1qjqe9dlu745drylkhs8ch6290lkntv2art8qyyp',
    rbfEnabled: false,
    fee: 792,
    broadcastTime: 1700029210,
    timestamp: 0,
    psbt: '',
    outAddrs: [
  ...
  final Transaction tx2 = Transaction(
    ...
    received: 997359,
    sent: 1000000,
    label: label.length > 1 ? label[1] : null,
    toAddress: 'tb1qnu4af4jycs8j43q7jhfnnglhxa6yk9lp58f4ls',
    fee: 141,
    broadcastTime: 1700030415,
    timestamp: 0,
    psbt: '',
    outAddrs: [
      Address(
  ...
  final Transaction tx3 = Transaction(
    ...
    received: 500000,
    sent: 0,
    label: label.length > 2 ? label[2] : null,
    toAddress: 'tb1qscvnchw8jgyu0383v6jhqsux6jyhh5zq52f2es',
    fee: 481,
    broadcastTime: 1700031019,
    timestamp: 0,
    psbt: '',
    outAddrs: [
      Address(
...
Wallet getWalletWith3Txns({List<String> label = const []}) {
  final externalAddressBook = [
    Address(
      address: 'tb1qnu4af4jycs8j43q7jhfnnglhxa6yk9lp58f4ls',
      kind: AddressKind.external,
  ...
  final Transaction tx1 = Transaction(
    txid: 'ca37042c41507326851792fd14a277edad58be4f92279ae49c78ea536e',
    received: 1000000,
    sent: 0,
    label: label.isNotEmpty ? label[0] : null,
    toAddress: 'tb1qjqe9dlu745drylkhs8ch6290lkntv2art8qyyp',
...
void getUnconfirmedTxns() {
  final Transaction unconfTx1 = Transaction(
    timestamp: 0,
    sent: 500,
    received: 1000,
    psbt:
        '',
    txid: '480d98fa8fa67709aa002665840bf45bdeed2182d4404fb3c7a9f0627237233a',
    toAddress: 'tb1qe6k6fv3e5unukdkxwmc7y5l36v7tushvg3yxm4',
    broadcastTime: 1700153924403,
    fee: 209,
    label: '',
    outAddrs: [
      Address(
        address: 'tb1qe6k6fv3e5unukdkxwmc7y5l36v7tushvg3yxm4',
        kind: AddressKind.external,
        state: AddressStatus.used,
      ),
    ].toList(),
